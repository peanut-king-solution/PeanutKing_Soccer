...
PeanutKingSoccerV3::PeanutKingSoccerV3(void) : actledPin(37),
                                               topBoardAddr(10),
                                               sensorBoardAddr(12),
...
void PeanutKingSoccerV3::motorSet(uint8_t mi, int16_t speed)
{
  ...
  if (targetSpeed[mi] > 0)
  {
    if (targetSpeed[mi] < 10)
      targetSpeed[mi] = 0;
    else if (targetSpeed[mi] < 25)
      targetSpeed[mi] = 25;
    else if (targetSpeed[mi] > 255)
      targetSpeed[mi] = 255;
  }
...
void PeanutKingSoccerV3::motorUpdate(void)
{
  ...
  {
    if (targetSpeed[mi] > currentSpeed[mi])
      currentSpeed[mi]++;
    else if (targetSpeed[mi] < currentSpeed[mi])
      currentSpeed[mi]--;
    else
      continue;
...
//     }
//     else
//     { // if ( btGestureCode==1 || btGestureCode==3 || btGestureCode==6 )
...
void PeanutKingSoccerV3::bluetoothRemote(void)
{
  ...
  if (Serial1.available())
  {
    ...
    if (msg[0] == 'Z') // light control
    {
      char subsequent[8] = {0};

      Serial1.readBytes(subsequent, 8); // LED commands have more bytes to indicate the color
      // Serial1.flush();

      char combined_color[9] = {msg[1], subsequent[0], subsequent[1], subsequent[2], subsequent[3], subsequent[4], subsequent[5], subsequent[6], subsequent[7]};

      // Serial.print("red light: ");
      String red_char = String(combined_color[0]) + String(combined_color[1]) + String(combined_color[2]);
      red = red_char.toInt();
      // Serial.print("red ");
      // Serial.println(red);
      String green_char = String(combined_color[3]) + String(combined_color[4]) + String(combined_color[5]);
      green = green_char.toInt();
      String blue_char = String(combined_color[6]) + String(combined_color[7]) + String(combined_color[8]);
      blue = blue_char.toInt();
    }
    else if(msg[0]==FORWARD || msg[0]==BACKWARD || msg[0]==LEFT || msg[0]==RIGHT){ // motor control

      char subsequent[11] = {0};
      Serial1.readBytes(subsequent, 11);

      int16_t m1speed = (String(msg[1])+String(subsequent[0])+String(subsequent[1])).toInt();
      if(m1speed >= 600){
        m1speed = (m1speed-900);
      }
      int16_t m2speed = (String(subsequent[2])+String(subsequent[3])+String(subsequent[4])).toInt();
      if(m2speed >= 600){
        m2speed = (m2speed-900);
      }
      int16_t m3speed = (String(subsequent[5])+String(subsequent[6])+String(subsequent[7])).toInt();
      if(m3speed >= 600){
        m3speed = (m3speed-900);
      }
      int16_t m4speed = (String(subsequent[8])+String(subsequent[9])+String(subsequent[10])).toInt();
      if(m4speed >= 600){
        m4speed = (m4speed-900);
      }
      ...
      // Serial.print(m3speed);Serial.print("  ");
      // Serial.print(m4speed);Serial.print("  ");

    }
    else if (msg[0]==CIRCLE || msg[0]==SQUARE) // used by rotation
    {
      char subsequent[2] = {0};
      Serial1.readBytes(subsequent, 2);
...
void PeanutKingSoccerV3::lcdSetup(void)
{
  ...
  write4bits(0x03 << 4);
  delayMicroseconds(4200); // Start in 8bit mode, try 4 bit mode, wait > 4.1ms
  write4bits(0x03 << 4);
  delayMicroseconds(150); // second try, wait > 4.1ms
  write4bits(0x03 << 4);
  delayMicroseconds(37); // third go!
  write4bits(0x02 << 4);
  delayMicroseconds(37);

  send(0b00101000, 0);
  delayMicroseconds(37); // Function Set - 4 bits(Still), 2 lines, 5x8 font
  send(0x08 | 0x04, 0);
  delayMicroseconds(37); // Initialize to default text direction
...
void PeanutKingSoccerV3::bluetoothRename(String name)
{ // Rename HC-05
...
void PeanutKingSoccerV3::bluetoothPrintName(void)
{ // Display HC05 name on LCD screen
  Serial1.flush();
  Serial1.print("AT+NAME?\r\n");
  while (!Serial1.available())
    ;
